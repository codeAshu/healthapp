...
public class ViewMemActivity extends ListActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		HealthApplication.fid=  Integer.parseInt(bundle.getString("famID")); //Fid needs to be set by Ashutosh's Code. It is actually the FAMILY ID.
		HealthApplication.flag=  bundle.getString("flag");
		appflag = HealthApplication.flag;
	...
	//	Toast.makeText(this, HealthApplication.flag, Toast.LENGTH_LONG).show();
		if(appflag.equals("doc"))
		{
			Intent i=new Intent( getApplicationContext(),NewVisitActivity.class);  //static fid will be having the data
			HealthApplication.mid = (adapter.getItem(position).getMid());		//static mid is also set now	
			Toast.makeText(this, Integer.toString(HealthApplication.mid), Toast.LENGTH_LONG).show();
			startActivity(i);
		}
		else
		{
			Intent i=new Intent( getApplicationContext(),DashboardActivity.class); //display questions
			startActivity(i);
			finish();
		}
	}
	private void setUpViews() {
		flag_p = (TextView)findViewById(R.id.flag);
	...
	private void setlist() {
		...
		if((fcheck.toCharArray())[8]!='A')
		{
			...
			flag_p.setText(docname+" has been assigned to your family.");
